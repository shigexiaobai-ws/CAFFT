...
totalRows = size(E, 1); % 获取总行数
fprintf('  文件 %s.csv 总共有 %d 行数据。\n', Name, totalRows); % 输出总行数
...
while(i<=size(E,1))%遍历整个记录
    ...
    %disp(class(Y(1,11)))
    %class(X(1,11))
    T=etime(datevec(string(Y(1,11))),datevec(string(X(1,11))));
    ...
    if(AX==AY)
            ...
            if(Total_Time>=Delta)
                if(size(R,1)>MINSIZE&&GPS_Zero<size(R,1)*0.75)
                    fprintf('  经纬度相同，累计时间超过阈值，正在写入文件 Road%d.csv\n', R_Index); % 输出写入文件信息
    ...
    if(T>MAXTime)
            if(size(R,1)>MINSIZE&&GPS_Zero<size(R,1)*0.75)
               fprintf('  时间间隔超过阈值，正在写入文件 Road%d.csv\n', R_Index); % 输出写入文件信息
               cell2csv([Name,'/Road',num2str(R_Index),'.csv'],R);
                 R_Index=R_Index+1;
            end
            GPS_Zero=0;
    ...
    % if(haversine(AX(1, 2), AX(1, 1), AY(1, 2), AY(1, 1))>MAXDIS)
            if(size(R,1)>MINSIZE&&GPS_Zero<size(R,1)*0.75)
                fprintf('  距离超过阈值，正在写入文件 Road%d.csv\n', R_Index); % 输出写入文件信息
                 cell2csv([Name,'/Road',num2str(R_Index),'.csv'],R);
                 R_Index=R_Index+1;
            end
            GPS_Zero=0;
