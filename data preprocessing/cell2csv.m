...
% % tic;C={'te','tm';5,[10000,1]);cell2csv([datestr(now,'MMSS') '.csv'],C);toc;
...
function x = StringX(x)
    ...
    % If string or char -> keep the value
    elseif ischar(x) || isstring(x)
        %No action needed        
    % 如果是 datetime 类型，转换为字符串
    elseif isa(x, 'datetime')
        x = datestr(x, 'yyyy-mm-dd HH:MM:ss'); % 或者使用你需要的格式
